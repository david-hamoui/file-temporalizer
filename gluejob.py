...
def convert_to_partitions(s3_objects):
    ...
    for item in s3_objects:
        ...
        joined_df = pd.concat([joined_df,data_frame],ignore_index=True)
    
    joined_df = joined_df.sort_values(by='created_at')

    grouped = joined_df.groupby(by=pd.Grouper(key='created_at',freq=parameter_dict[env['job_mode']][0].upper()))

    list_of_all_partitions = []
    for date,df in grouped:
        list_of_all_partitions.append([getattr(date,parameter_dict[env['job_mode']]),df.copy()])
...
def upload_partitions_to_s3(list_of_all_partitions):
    ...
    for date,df in list_of_all_partitions:
        ...
        s3.upload_fileobj(Fileobj=parquet_bytes,Bucket=env['bucket'], Key = f"{env['path']}/{env['table_name']}/company=Locaweb/new-folder/{env['table_name']}_{parameter_dict[env['job_mode']]}={date}/{env['table_name']}.parquet")
